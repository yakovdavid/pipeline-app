...
// aren't part of PortfolioStock, only StockQuote's Ambush-Radar-only
// fields, so they're untouched here). This is the SAME "one linear scalar
// applied to every price-like field" pattern the backend's TASE Agorot
// conversion and this app's deriveLocalValue already use (see
// @/utils/currency) — correcting the current price by X correctly
// corrects its 52-week high by X too, since both are drawn from the same
// (equally distorted) Yahoo data for that instrument.
//
// drawdownPct is deliberately NOT touched: it's a ratio between price and
// high52, and scaling both of those by the same factor leaves their ratio
// unchanged — recalibrating would be a no-op there, so this simply passes
// it through as-is via the spread.
export function calibrateQuote<Q extends { price: number; localPrice: number; high52: number | null }>(
  quote: Q,
